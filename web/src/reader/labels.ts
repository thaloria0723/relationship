...
//  灰滴(已退场/未出场)偏暗 → 月光淡银字 + 暗晕。样式见 src/reader/reader.css。
//
//  ⚠ 归属(2026-09-12):自验证页删除后**恢复为生产模块**(取自 05bce95),
//    待接入生产页面(宿主接线:每帧 place(投影位);投影用 viewer 的 screenOf)。
//    名称数据源未接 —— 接入前用占位文案。
...
/** 区间归一化:区间内 0→1、区间外钳制(自包含 —— 原实现从验证页驱动模块引入,
 *  验证页删除后内置;语义与 fxdriver/span01 一致) */
const span01 = (t: number, a: number, b: number): number =>

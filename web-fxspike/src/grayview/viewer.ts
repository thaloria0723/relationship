...
export interface FxSource {
  ...
  /** 桥抽出/回缩进度(0..1) */
  bridgeGrow(): number;
  /** 逐滴参数(传液滴索引,由实现判定谁演主角) */
  dropletFx(index: number): FxDropletParams;
  /** 雾团(凝结的「起雾」/ 死亡的「残留雾气」;cx/cy = 主角滴渲染坐标) */
  fogCue(
    index: number,
    cx: number,
    cy: number,
...
function mountViewer(
  ...
  const syncFog = (): void => {
    ...
    const d = engine.droplets.state;
    // 主角滴 = 索引 1(与 main.ts 的 HERO 一致);雾心略高于滴心(「空气中泛起」)。
  ...
  const syncBridges = (): void => {
    ...
    for (let k = 0; k < bridgeMax; k++) {
      ...
      const fxGrow = fxSource ? fxSource.bridgeGrow() : 1;
      ...
      if (bridgeKindAttr && bridgeStateAttr && bridgeGrowAttr) {
        ...
        const p = fxSource ? fxSource.bridgeFx(seedK) : null;
        const kind = fxSource ? fxSource.bridgeKind() : 0;
        const grow = fxSource ? fxSource.bridgeGrow() : 1;
        for (let v = 0; v < vertsPerBridge; v++) {
          const vi = base + v;
          ka[vi] = kind;
          ga[vi] = grow;
